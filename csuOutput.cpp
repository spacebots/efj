...
int isMachineLittleEndian() {
        ...
        if (*c == 0x78){ return 1;}//little endian
...
void efj::Database::writeSubspace(std::string output_file){
	...
	fprintf (file, "\n");
	  ...
	  fprintf (file, "BASIS_VALUE_COUNT  = %d\n", efj::Database::_eigenValues.rows());   /* basisDim */
	  fprintf (file, "BASIS_VECTOR_COUNT = %d\n", efj::Database::_eigenVectors.cols());
	  ...
	  /* write out the pixel count */
	    ...
	    for (i = 0; i < _eigenVectors->cols(); i++) {
	      for (j = 0; j < _eigenVectors->rows(); j++) {
	        writeDouble (file, _eigenVectors(j, i));
	      }
	    }
	    fclose (file);
